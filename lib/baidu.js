...
var Service = {
  ...
  upload: {
    url: 'https://pcs.baidu.com/rest/2.0/pcs/file?method=upload&' +
      'access_token=%s&path=%s'
  }
};
  ...
  if (this.session.accessToken) {
    ...
    this.quota(function(result, resp) {
      ...
      if (self.loginRequired) {
        baohe.processLogin(self.login.bind(self), callback);
      } else {
...
Baidu.prototype.login = function(username, password, callback) {
  var self = this;
